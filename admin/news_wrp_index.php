...
// FICHIER news_wrp_index.$phpExtJeu

	if (isset ($_POST['mod'])) {
		if (($fichier = fopen ('../news/html/index.html', "w+b"))!==false) {
			if (fwrite ($fichier, $_POST['contenu'])===false) {
				$template_main .= "Probleme à l'écriture de '../news/html/index.html'";
			}
			else 
			if (fclose ($fichier)===false)
				$template_main .= "Probleme à la fermeture de '../news/html/index.html'";
		}
		else die ("impossible d'ouvrir le fichier '../news/html/index.html' en ecriture");		
	} else {
$template_main .="		<center>
		<table width='750'>
		<tr><td><center><br />Modification de index.html</center></td></tr>
		<tr><td>
		<center><br />
...
$template_main .="
		...
		$template_main .= file_get_contents ('../news/html/index.html');
