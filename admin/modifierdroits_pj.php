...
$RCSfile: modifierdroits_pj.php,v $
*/
...
if($etape==2){
	if($MJ->aDroit($liste_flags_mj["ModifierMJ"]) && $MJ->aDroit($liste_flags_mj["ModifierDroitsMJauxPJs"]) && ($MJ->ID!= $id_cible ||$MJ_ID ==1)){
		$erreur="";
...
if($etape=="0"){
	$template_main .= "<div class ='centerSimple'><form action='".NOM_SCRIPT."' method='post'>";
	$template_main .= "Quel PJ voulez vous modifier les droits ?<br />";
	$SQL = "Select role_mj  as idselect, T1.nom as labselect from ".NOM_TABLE_REGISTRE." T1 where role_mj is not null ORDER BY T1.nom ASC";
